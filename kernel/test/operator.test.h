...
qb_suite(test_operator, "operator", {
    ...
    qb_describe("assign", {
        ...
        qb_test("INT <- INT", {
            ...
            block.data.__cpp_set(0, (uint8_t) 111);
            block.data.__cpp_set(1, (uint8_t) 222);

            auto data0 = block.data.__cpp_get<uint8_t>(0);
            auto data1 = block.data.__cpp_get<uint8_t>(1);
            qb_assert(*data0 == 111);
            qb_assert(*data1 == 222);
        ...
        qb_test("INT <- STR: explicit", {
            ...
            auto block = qb::mem::Block(type_def);
            block.data.__cpp_set(0, (uint8_t) 111);
            block.data.__cpp_set(1, (std::string) "222");

            auto data0 = block.data.__cpp_get<uint8_t>(0);
            auto data1 = block.data.__cpp_get<std::string>(1);
            qb_assert(*data0 == 111);
            qb_assert(*data1 == "222");
    ...
    qb_describe("math: dec", {
        ...
        qb_test("INT + INT", {
            ...
            auto block = qb::mem::Block(type_def);
            block.data.__cpp_set(0, (uint8_t) 11);
            block.data.__cpp_set(1, (uint8_t) 22);
