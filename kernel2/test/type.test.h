...
qb_suite(test_type, "type", {
    ...
    qb_describe("built-in", {
        ...
        qb_test("void", {
            auto type = solver.get(B_TYPE_VOID);
        ...
        qb_test("null", {
            auto type = solver.get(B_TYPE_NULL);
        ...
        qb_test("bool", {
            auto type = solver.get(B_TYPE_BOOL);
        ...
        qb_test("u8", {
            auto type = solver.get(B_TYPE_U8);
        ...
        qb_test("i8", {
            auto type = solver.get(B_TYPE_I8);
        ...
        qb_test("u16", {
            auto type = solver.get(B_TYPE_U16);
        ...
        qb_test("i16", {
            auto type = solver.get(B_TYPE_I16);
        ...
        qb_test("u32", {
            auto type = solver.get(B_TYPE_U32);
        ...
        qb_test("i32", {
            auto type = solver.get(B_TYPE_I32);
        ...
        qb_test("f32", {
            auto type = solver.get(B_TYPE_F32);
        ...
        qb_test("str", {
            auto type = solver.get(B_TYPE_STR);
        ...
        qb_test("ref", {
            auto type = solver.get(B_TYPE_REF);
    ...
    qb_describe("map", {
        ...
        qb_test("map{bool}", {
            auto tdx = solver.add_map(B_TYPE_BOOL);
        ...
        qb_test("map{u8}", {
            auto tdx = solver.add_map(B_TYPE_U8);
            auto type = solver.get(tdx);
        ...
        qb_test("map{f32}", {
            auto tdx = solver.add_map(B_TYPE_F32);
            auto type = solver.get(tdx);
        ...
        qb_test("map{str}", {
            auto tdx = solver.add_map(B_TYPE_STR);
            auto type = solver.get(tdx);
        ...
        qb_test("map{ref}", {
            auto tdx = solver.add_map(B_TYPE_REF);
            auto type = solver.get(tdx);
    ...
    qb_describe("struct", {
        ...
        qb_test("struct{bool,u8,f32}", {
            ...
            qb_assert(type->schema.of_struct.fields[0] == solver.get(B_TYPE_BOOL))
            qb_assert(type->schema.of_struct.fields[1] == solver.get(B_TYPE_U8))
            qb_assert(type->schema.of_struct.fields[2] == solver.get(B_TYPE_F32))
        })
        ...
        qb_test("struct{bool,u8,f32,str}", {
            ...
            qb_assert(type->schema.of_struct.fields[1] == solver.get(B_TYPE_U8))
            qb_assert(type->schema.of_struct.fields[2] == solver.get(B_TYPE_F32))
            qb_assert(type->schema.of_struct.fields[3] == solver.get(B_TYPE_STR))
        })
        ...
        qb_test("struct{bool,u8,f32,str,ref}", {
            ...
            qb_assert(type->schema.of_struct.fields[2] == solver.get(B_TYPE_F32))
            qb_assert(type->schema.of_struct.fields[3] == solver.get(B_TYPE_STR))
            qb_assert(type->schema.of_struct.fields[4] == solver.get(B_TYPE_REF))
        })
    ...
    qb_describe("event", {
        ...
        qb_test("event{bool}", {
            auto tdx = solver.add_event(B_TYPE_BOOL);
            auto type = solver.get(tdx);
        ...
        qb_test("event{u8}", {
            auto tdx = solver.add_event(B_TYPE_U8);
            auto type = solver.get(tdx);
        ...
        qb_test("event{f32}", {
            auto tdx = solver.add_event(B_TYPE_F32);
            auto type = solver.get(tdx);
        ...
        qb_test("event{str}", {
            auto tdx = solver.add_event(B_TYPE_STR);
            auto type = solver.get(tdx);
        ...
        qb_test("event{ref}", {
            auto tdx = solver.add_event(B_TYPE_REF);
            auto type = solver.get(tdx);
    ...
    qb_describe("vec", {
        ...
        qb_test("vec{bool}", {
            auto tdx = solver.add_vec(B_TYPE_BOOL);
            auto type = solver.get(tdx);
        ...
        qb_test("vec[u8]", {
            auto tdx = solver.add_vec(B_TYPE_U8);
            auto type = solver.get(tdx);
        ...
        qb_test("vec{f32}", {
            auto tdx = solver.add_vec(B_TYPE_F32);
            auto type = solver.get(tdx);
        ...
        qb_test("vec{str}", {
            auto tdx = solver.add_vec(B_TYPE_STR);
            auto type = solver.get(tdx);
        ...
        qb_test("vec{ref}", {
            auto tdx = solver.add_vec(B_TYPE_REF);
            auto type = solver.get(tdx);
    ...
    qb_describe("complex map", {
        ...
        qb_test("map{map{bool}}", {
            auto tdx0 = solver.add_map(B_TYPE_BOOL);
            auto tdx1 = solver.add_map(tdx0);
        ...
        qb_test("map{event{u8}}", {
            auto tdx0 = solver.add_event(B_TYPE_BOOL);
            auto tdx1 = solver.add_map(tdx0);
            auto type = solver.get(tdx1);
        ...
        qb_test("map{vec[u8]}", {
            auto tdx0 = solver.add_vec(B_TYPE_BOOL);
            auto tdx1 = solver.add_map(tdx0);
            auto type = solver.get(tdx1);
    ...
    qb_describe("complex event", {
        ...
        qb_test("event{map{bool}}", {
            auto tdx0 = solver.add_map(B_TYPE_BOOL);
            auto tdx1 = solver.add_event(tdx0);
            auto type = solver.get(tdx1);
